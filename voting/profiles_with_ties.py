...
class ProfileWithTies(object):
    ...
    def plurality_scores(self): 
        '''calcualte the plurality scores of each candidate.   The plurality score 
        is the number of voters that rank the candidate *uniquely* in first place.'''
        first_place_cands_num = [(r.first(),c) for r,c in zip(self.rankings, self.rcounts)]
        return {c:sum([cn[1] for cn in first_place_cands_num if cn[0] == [c]]) for c in self.candidates}
